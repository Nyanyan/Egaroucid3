...
inline search_result search(const board b){
    ...
    if (b.n + depth + 1 < hw2){
        for (int use_depth = 3; use_depth <= depth; ++use_depth){
            for (i = 0; i < canput; ++i){
            ...
            if (canput)
                sort(nb.begin(), nb.end(), move_ordering);
            g = -nega_scout(&nb[0], strt, 0, use_depth, -beta, -alpha);
            alpha = max(alpha, g);
            tmp_policy = nb[0].policy;
            for (i = 1; i < canput; ++i){
                g = -nega_alpha_ordering(&nb[i], strt, 0, use_depth, -alpha - window, -alpha);
                if (alpha < g){
                    g = -nega_scout(&nb[i], strt, 0, use_depth, -beta, -g);
                    if (alpha < g){
                        alpha = g;
...
int main(){
    ...
    while (true){
        cin >> ai_player;
        b.n = input_board(b.b, direction);
        b.p = ai_player;
        double v = search(b).value;
        cerr << v << endl;
        cout << v << endl;
    }
    return 0;
