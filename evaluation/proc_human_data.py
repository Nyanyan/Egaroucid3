...
ai = subprocess.Popen('./ai.out'.split(), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
...
def collect_data(num, s):
    ...
    while True:
        ...
        if rv.check_pass() and rv.check_pass():
            break
        if len(s) - 16 * 2 <= idx:
            board_proc = str(rv.player) + '\n'
            for i in range(hw):
                for j in range(hw):
                    board_proc += '0' if rv.grid[i][j] == 0 else '1' if rv.grid[i][j] == 1 else '.'
                board_proc += '\n'
            ai.stdin.write(board_proc.encode('utf-8'))
            ai.stdin.flush()
            try:
                y, x = [int(i) for i in ai.stdout.readline().decode().split()]
                records += chr(x + ord('a')) + str(y + 1)
                if rv.move(y, x):
                    print('error')
                    exit()
                idx += 2
    ...
    with open('third_party/records_proc/' + digit(num, 7) + '.txt', 'a') as f:
        f.write(records + '\n')
